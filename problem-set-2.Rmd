...
ps_formula <- physically.inactive ~ sex + smoking.status + 
  ...
  # income.poverty.ratio.missingind + dietary.adequacy.missingind

# fit a (logistic regression) propensity model
ps_model <- rigr::regress("odds", data = nhanes, ps_formula)
# calculate propensity scores and inverse probability weights
nhanes$ps.score = predict(ps_model, type = "response")

# truncate propensity scores that are outside range of scores from control group
score_range <- nhanes %>% 
...
Using one of your matchings (e.g., your pair matching) to perform a sensitivity analysis. Up to what value of $\Gamma$ is there still evidence that being physically active causes you to live longer?
...
senWilcox(diff, gamma = 1, conf.int = F, alpha = 0.05, alternative = "less")
senWilcox(diff, gamma = 1.6, conf.int = F, alpha = 0.05, alternative = "less")
```
...
X <- model.matrix(glm(ps.formula, family = binomial, data = nhanes))
X <- X[,-1]
